...
function WorldMap(config) {
    ...
    var inactiveCountryStroke = 'grey';
    var inactiveCountryStrokeWidth = 2;

    var activeCountryOpacity = 1.0;
    var activeCountryFill = 'white';
    var activeCountryStroke = 'grey';
    var activeCountryStrokeWidth = 6;
    ...
    for (var c = 0; c < countries.length; c++) {
        ...
        $(svg.root()).bind('mousemove',
            ...
            function (path) {
                ...
                $(g).attr('stroke-width', config.activeCountryStrokeWidth);
                ...
                if (config.showCountryBoxOnMouserEnter) {
                    ...
                    search_country=c_names.indexOf(config.countryId)
                    if(search_country>-1){
	                    var name = getCountryName(config.countryId).split(',')[0];
    	                txt[0].textContent = name.toUpperCase()+', '+num_exp[search_country].toString()+'('+sem_exp[search_country]+')';	                    
...
//                 }, 1000);
        ...
        function (path) {
            ...
            $(g).attr('stroke-width', config.inactiveCountryStrokeWidth);
            $('#' + config.id + 'box').stop();
...
function createTextBox(config, svg) {
    ...
    var pathString = 'M-100,-10 L60,-10 L60,-30 L-100,-30 L-100,-10';

    svg.path(g, pathString, { fill: '#000', stroke: '#000', strokeWidth: 1, transform: 'translate(5,5)', opacity: 0.1 });
